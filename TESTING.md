...
| 11.4 | Manually Tab (or click) into a text field (Full name, Email, Phone, or Notes) while scan mode is on, then wait several seconds. | Focus **stays** in that field — scanning self-pauses so you have unlimited time to type. |
| 11.5 | Tab or click out of that text field to any other control. | Scanning resumes advancing from wherever focus now is. |
